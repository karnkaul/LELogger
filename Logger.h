...
#define LOGIF_E(bCond, x, ...)                 \
	if (bCond)                                 \
	{                                          \
		LOG_SEVERITY(x, Error, ##__VA_ARGS__); \
	}
#define LOGIF_W(bCond, x, ...)                   \
	if (bCond)                                   \
	{                                            \
		LOG_SEVERITY(x, Warning, ##__VA_ARGS__); \
	}
#define LOGIF_I(bCond, x, ...)                \
	if (bCond)                                \
	{                                         \
		LOG_SEVERITY(x, Info, ##__VA_ARGS__); \
	}
